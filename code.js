...
// 文件名后缀
var fileNameSuffix = '_model.dart'
  ...
  ;(function init() {
    ...
    function generate() {
      ...
      let objToDart = (jsonObj, prefix, baseClass) => {
        ...
        let className = `${prefix}${uppercaseFirst(baseClass)}${classSuffix}`
        ...
        for (let key in jsonObj) {
          if (jsonObj.hasOwnProperty(key)) {
            ...
            if (typeof element === 'object') {
              let subClassName = `${uppercaseFirst(key)}${classSuffix}`
              ...
              if (Array.isArray(element)) {
                ...
                lines.unshift(objToDart(element, '', key))
      ...
      rootClass = rootClass.length > 0 ? rootClass.charAt(0).toUpperCase() + rootClass.slice(1) : 'My'
      ...
      prefixDartCode += `${objToDart(jsonObj, rootClass, '')}`
